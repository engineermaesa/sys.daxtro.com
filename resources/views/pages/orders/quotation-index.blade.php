...
<section class="min-h-screen sm:text-xs! lg:text-sm!">
    ...
    <div class="pt-4">
        <div class="flex items-center gap-2">  
            ...
            <h1 class="text-[#115640] font-semibold lg:text-2xl text-lg">Quotation</h1>
        </div>
        <p class="mt-1 text-[#115640] lg:text-lg text-sm">List Quotations</p>
    </div>
    ...
    <div class="mt-4 rounded-lg border-[#D9D9D9]">
        ...
        <div
            ...
            <div class="border border-[#D5D5D5] rounded-lg grid grid-cols-1 h-full">
                @foreach (['warm'] as $tab)
                {{-- NAVIGATION STATUS --}}
                <div data-status="{{ $tab }}"
                    class="text-center cursor-pointer py-2 h-full nav-leads-active flex items-center justify-center">
                    <p class="text-[#083224]">
                        Warm Stage
                        <span id="nav-count-{{ $tab }}" class="span-warm">
                            {{ '(' . $leadCounts['warm'] . ')' }}
                        </span>
                    </p>
                </div>
...
<script>
    ...
    function renderWarmSummary(warm = {}) {
        $("#summary-warm-total").text(warm.total || 0);
        $("#summary-warm-no-quotation").text(`( ${warm.no_quotation || 0} No Quotation )`);

        $("#summary-warm-total-pending").text(warm.approval_status || 0);
        $("#summary-warm-pending-count").text(`( ${warm.pending || 0} Pending & `);
        $("#summary-warm-rejected-count").text(`${warm.rejected || 0} Rejected )`);

        $("#summary-warm-quotations").text(warm.quotation_published || 0);
    }
    ...
    function updatePagerUI(tab, totalItems) {
        ...
        const endIdx = Math.min(totalItems || 0, (page - 1) * pageSize + pageSize);

        if (showing) {
            showing.innerText = `Showing ${startIdx}-${endIdx} of ${totalItems || 0}`;
        }
    ...
    function switchTab(statusTarget) {
        activeTabState = statusTarget || 'warm';

        $('[data-status-wrapper]').addClass('hidden');
        $(`[data-status-wrapper="${activeTabState}"]`).removeClass('hidden');

        $('.nav-leads-active').removeClass('border-b-2 border-b-[#115640] text-white active-nav');
        $(`.nav-leads-active[data-status="${activeTabState}"]`).addClass('border-b-2 border-b-[#115640] active-nav');

        const allCards = ['forWarmCardsCounts'];
        ...
        const targetCard = cardMap[activeTabState];
    ...
    async function initTable(selector, route) {
        ...
        try {
            ...
            updateBadgeCounts();
            updatePagerUI(selector, result.total || 0);
            ...
            if (result.data && result.data.length > 0) {
                result.data.forEach(row => {
                    const statusContent = row.meeting_status ?? '-';

                    tbody.innerHTML += `
                        <tr class="border-t border-t-[#D9D9D9] text-[#1E1E1E]">
                            <td class="hidden">${row.id ?? ''}</td>
                            <td class="p-1 md:p-2 lg:p-3">${row.claimed_at ?? '-'}</td>
                            <td class="p-1 md:p-2 lg:p-3">${row.lead?.name ?? row.name ?? '-'}</td>
                            <td class="p-1 md:p-2 lg:p-3">${row.sales?.name ?? row.sales_name ?? 'Not Set'}</td>
                            <td class="p-1 md:p-2 lg:p-3">${row.lead?.phone ?? row.phone ?? 'Not Set'}</td>
                            <td class="p-1 md:p-2 lg:p-3">${row.lead?.source?.name ?? row.source ?? 'Not Set'}</td>
                            <td class="p-1 md:p-2 lg:p-3">${row.lead?.needs ?? row.needs ?? 'Not Set'}</td>
                            <td class="p-1 md:p-2 lg:p-3">${row.lead?.region?.name ?? row.lead?.alternate_location?.region_name ?? row.city_name ?? 'Not Set'}</td>
                            <td class="p-1 md:p-2 lg:p-3">${row.lead?.region?.regional?.name ?? row.lead?.alternate_location?.regional_name ?? row.regional_name ?? 'Not Set'}</td>
                            <td class="p-1 md:p-2 lg:p-3">${row.lead?.province ?? row.lead?.alternate_location?.province_name ?? row.province_name ?? 'Not Set'}</td>
                ...
                tbody.innerHTML = `
                    <tr>
                        <td colspan="12" class="text-center p-3 text-[#1E1E1E]">
                            No data available
                        </td>
                    </tr>
                `;
            ...
            if (tbody) {
                tbody.innerHTML = `
                    <tr>
                        <td colspan="12" class="text-center p-3 text-red-600">
                            Failed to load data
                        </td>
                    </tr>
                `;
            ...
            data.forEach(function(item) {
                rows += `
                    <tr class="border-t border-t-[#D9D9D9]">
                        ...
                        <td class="lg:p-3 sm:p-1">
                            ${item.attachment ? `<a href="${item.attachment}" class="btn btn-sm btn-outline-secondary">Download</a>` : '-'}
                        </td>
