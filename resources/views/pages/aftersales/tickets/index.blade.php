    ...
    <div class="pt-4">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('aftersales.pages.tickets.create') }}" class="bg-[#115640] text-white px-4 py-2 rounded-lg hover:bg-[#0d4633] transition-colors flex items-center gap-2">
                <i class="bi bi-plus-lg"></i> Tambah Ticket
            </a>
        </div>
            ...
            <table class="w-full">
                <thead class="text-[#1E1E1E]">
                    <tr class="border-b border-b-[#D9D9D9]">
                        ...
                        <th class="p-3 text-left uppercase text-xs">Mesin</th>
                        <th class="p-3 text-left uppercase text-xs">Teknisi</th>
                        <th class="p-3 text-left uppercase text-xs">Provinsi</th>
                        <th class="p-3 text-left uppercase text-xs">Kota</th>
                        <th class="p-3 text-left uppercase text-xs">Desk.</th>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content rounded-2xl! overflow-hidden border-0!">
            ...
            <div class="modal-body px-6! pb-6! pt-0!">
                ...
                <label class="uppercase text-[11px] text-[#757575] font-semibold tracking-wide">Deskripsi Problem</label>
...
<script>
    ...
    function renderTicketRows(rows) {
        ...
        if (!rows || rows.length === 0) {
            $body.html('<tr><td colspan="15" class="text-center p-4 text-[#757575]">No tickets found</td></tr>');
            return;
        }
        ...
        const html = rows.map(row => {
            ...
            <tr class="border-t border-t-[#D9D9D9] align-top">
                ...
                <td class="p-3">${escapeHtml(customer.province)}</td>
                <td class="p-3">${escapeHtml(customer.city)}</td>
                <td class="p-3 max-w-[160px]">
                    ...
                    <button type="button" class="btn-ticket-detail text-[#115640] text-xs underline cursor-pointer"
                        data-ticket='@json([])'>- Lihat Detail</button>
                </td>
    ...
    function openTicketDetail(ticket) {
        ...
        $('#ticketDetailSubtitle').text(`${customer.name || '-'} · ${machine.machine_name || '-'}`);
