...
<script>
    ...
    function getActiveStatusTab() {
      const activeNav = document.querySelector('.nav-leads-active.border-b-2');
      return activeNav ? activeNav.getAttribute('data-status') : 'all';
    }

    function reloadActiveStatusTab() {
      reloadTab(getActiveStatusTab());
    }
...
$(function () {
  ...
  function applyRangeToFilter(picker, labelSelector, defaultLabel, filterKey) {
  ...
  $('#applyClaimedDate').on('click', function () {
    const ok = applyRangeToFilter(
      claimedPicker,
      '#claimedDateLabel',
      'Claimed At',
      'filter_by_claimed_at'
    );
    if (!ok) return;
