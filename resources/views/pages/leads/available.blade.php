...
<div class="min-h-screen sm:text-xs! lg:text-sm!">
  ...
  <section class="bg-white rounded-lg mt-3">
    ...
    <div class="lg:flex lg:px-3 lg:py-4 gap-3 grid grid-cols-1 px-2 py-3">
        ...
        {{-- BUTTON EXCEL EXPORT --}}
        <button id="btnExport" class="lg:w-1/4! bg-[#115640] text-white font-semibold rounded-lg cursor-pointer hidden lg:inline!" type="button">
          <i class="bi bi-download me-1"></i> Export Excel
        </button>
    </div>
      ...
      <table id="availableLeadsTable" class="w-full table-fixed">
        <thead class="border-b border-b-[#CFD5DC] border-t border-t-[#CFD5DC]">
          <tr>
            ...
            <th class="font-bold text-left lg:p-3 sm:p-1">Actions</th>
          </tr>
        </thead>
                  ...
                  tbody.append(`
                      <tr class="border-b border-b-[#CFD5DC]">
                          ...
                          <td class="lg:p-3 sm:p-1">${row.segment_name ?? ''}</td>
                            <td class="py-3 xl:flex! xl:items-center! xl:justify-start! xl:gap-3! grid grid-cols-1 gap-2 ">${row.actions ?? ''}</td>
                      </tr>
                  `);
              });
          }
